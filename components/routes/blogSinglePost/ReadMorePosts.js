...
const ReadMorePosts = ({ listPostsItems, listLinksToImg }) => {
  const [shuffledArray, setShuffledArray] = useState(
    helpersFunctionShuffleArray(listPostsItems),
  )
  const [showFewPosts, setShowFewPosts] = useState(
    shuffledArray.splice(listPostsItems.length - 3),
  )
        ...
        <Link href={`/blog/${item['ID Year']}`}>
          ...
          >
            ...
            <TitleAndText variant={'textSmall'} mb={20} textAlign={'left'}>
              {index}Over the past {item.Population} few months, we’ve added
              several new features to SaaS Invoicing to help any business get
              paid faster and streamline their collection workflows.
              {item.Population}
            </TitleAndText>
            <TitleAndText variant={'textSmall'} mb={20} textAlign={'left'}>
