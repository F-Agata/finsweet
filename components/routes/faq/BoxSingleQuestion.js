    ...
    return (
        <StyledBoxSingleQuestion>
            <Box
                ...
                // p={'20px  0 0 20px'}
            >
                <TitleAndText variant={'title4'} textAlign={'left'}>{question}</TitleAndText>
                <Box
                    pl={40}>
                    {!isOpen && <StyledAiOutlinePlus onClick={()=>setIsOpen(!isOpen)} />}
                    {isOpen && <StyledAiOutlineMinus onClick={()=>setIsOpen(!isOpen)} />}
                </Box>
            </Box>
            {isOpen && <TitleAndText variant={'textSmall'} p={'20px 40px'}>{smallDescription}</TitleAndText>}
        </StyledBoxSingleQuestion>
)}
